...
fn test_hmac_sha384_gold_data() {
    for data in HMAC_SHA384_GOLD_DATA {
        assert_mac_eq!(Hmac<Sha384>, data.key, data.message, data.expected);
    }
}
...
fn test_hmac_sha384_million_a() {
    ...
    let result = <Hmac<Sha384>>::compute(key, &message);
...
fn test_hmac_sha384_multi_update() {
    let key = b"Jefe";
    let mut mac = <Hmac<Sha384>>::new(key);
...
fn test_hmac_sha384_vec_methods() {
    let result_vec = <Hmac<Sha384>>::compute(b"abc", b"abc").as_ref().to_vec();
    let mut mac = <Hmac<Sha384>>::new(b"abc");
    mac.update(b"abc");
...
fn test_hmac_sha384_stress() {
    stress_test_mac::<Hmac<Sha384>>(1000);
}